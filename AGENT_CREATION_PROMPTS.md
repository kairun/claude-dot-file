...
Shared memory: Before tracking plans, read .kairun/working-memory.md to understand recent changes, completed work, and key decisions that may affect the plan. DO NOT duplicate information already captured in working memory - working memory is backward-looking (what happened, decisions made, discoveries) while your plans-and-todos file is forward-looking (what needs to happen, task roadmap, priorities). Cross-reference completed items from working memory when updating plan status, but focus your file on future work and implementation tracking. DO NOT edit working-memory.md - only the working-memory-manager agent should modify it. You manage .kairun/plans-and-todos.md exclusively.
